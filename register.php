...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!preg_match("/^\+?\d{1,4}?[-.\s]?\(?\d{1,4}?\)?[-.\s]?\d{1,9}([-.\s]?\d{1,9})?$/", $phone)) {
    ...
    if($confirm_password != $password){
        $confirm_password_error = "Password & Confirm Password do not match";
        $error = true;
    }
    ...
    if(!$error){
        ...
        $statement->bind_param('sssssss', $first_name, $last_name, $email, $phone, $address, $password_hash, $created_at);
        $statement->execute();
